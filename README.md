## Introduction


<b>Discipline | <b>Computer Science and Engineering
:--|:--|
<b> Lab | <b> Machine Learning Virtual Lab
<b> Experiment|     <b> EXP-1: Data Preprocessing and Feature Engineering

### About the Experiment 

This experiment focuses on performing data preprocessing and feature engineering on the Titanic dataset. It covers essential techniques such as handling missing values, normalizing numerical features, encoding categorical variables, creating new meaningful features, and visualizing the data. These steps are critical for improving data quality and preparing it for effective machine learning model development.

<b>Name of Developer | <b> Rohit
:--|:--|
<b> Institute | <b> Dayalbagh Educational Institute
<b> Email id|     <b> 3337.rohit64@gmail.com
<b> Department | Electrical Engineering

### Contributors List

SrNo | Name | Faculty or Student | Department| Institute | Email id
:--|:--|:--|:--|:--|:--|
1 | Dr. A. Charan Kumari | Faculty | Electrical Engineering | Dayalbagh Educational Institute | charankumari@dei.ac.in
2 | Dr. K. Srinivas | Faculty | Electrical Engineering | Dayalbagh Educational Institute | ksrinivas@dei.ac.in
3 | Amoli Satsangi | Student | Electrical Engineering | Dayalbagh Educational Institute | amolisatsangi6@gmail.com
4 | Sumir Singh | Student | Electrical Engineering | Dayalbagh Educational Institute | sumirsingh19@gmail.com


